...
jsonData.results.forEach(monster => {
    monsterNames.push(monster.name);
});
*/


// Extract monster names into an array
let monsterNames = jsonData.results.map(monster => monster.name);

// Get your select element
let selectElement = document.getElementById("monsterSelect"); 
...
defaultOption.text = "Choose a monster";
selectElement.add(defaultOption, selectElement[0]);

// Add each monster name as an option
