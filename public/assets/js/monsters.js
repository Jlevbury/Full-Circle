...
const monsterSelect = document.querySelector('#monsterSelect');
const searchButton = document.querySelector('input[type="submit"]');

searchButton.addEventListener('click', function(e) {
    e.preventDefault(); 
    const monsterName = monsterSelect.value;  
    if (monsterName && monsterName !== "Choose a monster") {
        getData(monsterName);
    } else {
        resultLocation.textContent = "Please select a monster.";
    }
});
...
populateMonsterDropdown(jsonData.results);

searchButton.addEventListener('click', function(e) {
    e.preventDefault(); 
    const monsterIndex = monsterSelect.value;
    if (monsterIndex) {
        getData(monsterIndex);
    } else {
        resultLocation.textContent = "Please select a monster.";
    }
});

function populateMonsterDropdown(monsters) {
    // clear any existing options
    monsterSelect.innerHTML = '';

    // create default option
    let defaultOption = document.createElement('option');
    defaultOption.textContent = "Choose a monster";
    defaultOption.value = "";
    monsterSelect.appendChild(defaultOption);

    // add each monster to dropdown
    monsters.forEach(monster => {
        let option = document.createElement('option');
        option.textContent = monster.name;
        option.value = monster.index; // use index as the value
